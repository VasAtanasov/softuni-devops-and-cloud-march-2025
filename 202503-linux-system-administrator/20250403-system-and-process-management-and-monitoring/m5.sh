...
if [ $? -eq 0 ]; then pts=$((pts+1)); fi
# check 2
systemctl show sshd.service -p After &> /tmp/1.txt
diff /tmp/1.txt /home/$tu/sshd-after.txt &> /dev/null
if [ $? -eq 0 ]; then pts=$((pts+1)); fi
# decision
if [ $pts -ge 1 ]; then tr='PASS'; else tr='ERROR'; fi
echo '... '$tr
